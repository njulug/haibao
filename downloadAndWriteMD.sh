...
do
    ...
    print $url
    a=`curl $url`
    j=`echo $a |grep "<span class='Article_Title'>"|grep "\/.*.htm" -o`
    ...
    do    
        ...
        #echo $url11$j1
        #echo $url1       
		title=`curl -s $url1 |grep read|grep -o "{.*}"|grep -E  "title.*\'" -o|sed "s/.jpg'}/ /g"|sed "s/.jpg'/ /g"|sed "s/title':'//g"|sed "s/海报模板-//g"|sed "s/海报模板//g"|sed "s/.JPG'//g"| sed "s/[ \t ]*$//g"|sed "s/ /_/g"|sed "s/（海报印制）//g"`
